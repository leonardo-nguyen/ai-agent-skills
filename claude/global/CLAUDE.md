...
Cross-project defaults for workflow, communication, and code quality. Project-level CLAUDE.md files may extend or override these.

---

## 1. AGENT ORCHESTRATION PROTOCOL (AUTONOMOUS DUAL-ROLE EXECUTION)
...
1. **CONTINUOUS SYNC GUARDRAIL (CRITICAL & NON-NEGOTIABLE)**:
   ...
   * You are FORBIDDEN from finishing your response or outputting completion text after code changes until physical files in the project's plans folder are updated on disk.
...
2. **PHYSICAL FILE PERSISTENCE & CREATION**:
   ...
   * Step 1: Create a physical dated plans folder (e.g. `/.docs/plans/[yyyyMMddhhmm]-[feature-name]/`) and write a `[feature-code]-plan.md` (Objectives, Logic/Edge Cases, Validation Strategy). Wait for User approval.
...
## 2. CORE QUALITY & EXECUTION RULES
...
Before generating or applying any code changes, you MUST internally verify:
1. Coding conventions and existing utilities in the target project are reused.
2. No duplicate variables, types, or functions are created.
3. UI strictly conforms to the project's design system.
4. Response tone is clear, natural, concise, and non-robotic.
